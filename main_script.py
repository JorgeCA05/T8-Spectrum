...
URL_forma_de_onda = url_generator('https://lzfs45.mirror.twave.io/lzfs45/rest/','waves',
...
URL_espectro_t8 = url_generator('https://lzfs45.mirror.twave.io/lzfs45/rest/','spectra',
                                 'LP_Turbine', 'MAD31CY005', 'AM1', fecha_hora)
# Hacemos el request con la URL generada con la función url_generator
...
#tratar de obetener el factor de conversion en la configuracion

def get_factors(url, usuario, contraseña):
    respuesta = requests.get(url, auth=(usuario, contraseña))
    if respuesta.status_code == 200:
        data = respuesta.json()
        factors = [unit['factor'] for unit in data['units']]
        return factors
    else:
        print(f"Error al acceder a la API: {respuesta.status_code}")
        return None



url_base='https://lzfs45.mirror.twave.io/lzfs45/rest/confs/0'
factors = get_factors(url_base, usuario, contraseña)
if factors:
...
plt.plot(freq_t8_filtrada, sp_filtrada, label='Espectro T8', linestyle='--')
